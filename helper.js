export function setColor(num) {
  // this function decides which color will ball have
...
export function checkTickets(round, drawnNumbers, tickets) {
  ...
  const checkedTickets = tickets.map((ticket) => {
    if (ticket.round === round) {
      if (
        ticket.numbers.every((num) => drawnNumbersOnlyNumbers.includes(num))
      ) {
        const positionOfLastDrawnBall = lastPos(
          drawnNumbersOnlyNumbers,
          ticket.numbers
        );
        const amountPaid =
          drawnNumbers[positionOfLastDrawnBall - 1].quota * ticket.amount;
        return {
          ...ticket,
